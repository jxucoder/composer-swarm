...
Composer Swarm uses JSONL for worker events. JSONL is easy for every CLI agent to emit and easy for host
...
```json
{"type":"started","taskId":"task_01","candidateId":"task_01-builder-a","agentId":"composer-builder-a","timestamp":"2026-05-22T19:00:00.000Z"}
```
...
```bash
composer-swarm adapter run composer-builder-a --task-file .composer-swarm/state/tasks/task_01-builder-a.json
```
